...
//      $this->debug($data_privilege, true);
    ...
    if(!$this->input->post(NULL, TRUE)){
      ...
      $file = $this->global_models->get_field('m_users', 'signature', array("id_users" => $id));
	  ...
	  $this->template->build('add-new', 
        array(
			  ...
			  'signature_link'=>site_url("file/index1/m_users/{$id}"),
	  ...
	  $config['upload_path'] = './files/users/signature/';
      ...
      if($_FILES['signature']['name']){
        if (  $this->upload->do_upload('signature')){
          ...
          print "<br /> <a href='".site_url("users/add-new/$id")."'>Back</a>";
          die;
        }
      }
...
//          $users_channel = $this->global_fungsi->curl_mentah($kirim, URLSERVER."json/json-midlle-system/set-users");
...
//          }
	...
	public function add_users_eksternal(){
    $pst = $this->input->post();
    if($pst['id_detail']){
      $kirim = array(
        'name'            =>  $pst["name"],
        ...
        'id_users'        =>  $pst["id_detail"],
      );
      if($pst["pass"]){
        if($pst["pass"] == $pst["repass"]){
          $kirim['pass'] = $this->encrypt->encode($pst["pass"]);
        }
      }

      $users_channel = $this->global_variable->curl_mentah($kirim, URLSERVER."json/set-users");
      $data_array = json_decode($users_channel);
      if($data_array->status == 2){
        unset($kirim['users']);
        unset($kirim['password']);
        unset($kirim['id']);
        $hasil_in = $this->global_models->update("m_users", array("id_users" => $pst["id_detail"]), $kirim);
      }
    }
    else{
      if($pst['pass'] == $pst['repass']){
        ...
        $users_channel = $this->global_variable->curl_mentah($kirim, URLSERVER."json/set-users");
        $data_array = json_decode($users_channel);
//        $this->debug($users_channel, true);
        $kirim_company = array(
          'users'             => USERSSERVER,
          'password'          => PASSSERVER,
          'id_users'          => $data_array->id_users,
          'id'                =>  $this->session->userdata('id'),
          'id_master_company' => $pst['id_master_company']
        );
        $company = $this->global_variable->curl_mentah($kirim_company, URLSERVER."json/set-users-company");
    ...
    redirect ("set/users-company/{$pst['id_master_company']}");
  }
...
//	}
  ...
  function index(){
    ...
    if($pst){
      $set_sess = array(
        "users_name"         => $pst['name'],
        "users_id_privilege" => $pst['users_id_privilege'],
    ...
    if($this->session->userdata("users_status") OR $this->session->userdata("users_status") === "0"){
...
//      $where .= " AND A.id_status_user = '{$this->session->userdata("users_status")}'";
//    }
//    
    ...
    $status = array(
        NULL => "<span class='label label-success'>Active</span>",
  ...
  function import_users(){
    $users = $this->global_models->get_query("SELECT A.*, B.id_privilege"
      . " FROM m_users AS A"
    ...
    foreach($users AS $us){
      $kirim = array(
          ...
          'pass'            =>  $us->pass,
          'email'           =>  $us->email,
          'id_status_user'  =>  $us->id_status_user,
          'id_privilege'    =>  $us->id_privilege,
          "create_by_users" =>  $us->create_by_users,
