...
//      $this->debug($_FILES, true);
      ...
      if($_FILES['file_promo']['name']){
        if (  $this->upload->do_upload('file_promo')){
          $data_promo = array('upload_data' => $this->upload->data());
        }
        else{
          print $this->upload->display_errors();
          print "<br /> <a href='".site_url("haji/master-haji/add-new-haji/".$id_website_haji)."'>Back</a>";
      ...
      if($pst['id_detail']){
        ...
        if($data_promo['upload_data']['file_name']){
          $kirim['file_promo'] = $data_promo['upload_data']['file_name'];
        }
