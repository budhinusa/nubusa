...
//      }
  ...
  public function get_privilege($key, $param){
    $hasil = $this->get($key);
    $data = explode("[".$param."]", $hasil);
    return $data[1];
  }
...
//      }
  ...
  public function clear_free($key, $type, $privilege = "", $content = ""){
    $data = explode("[{$type}]", $this->get($key));
    $data[1] = str_replace($privilege."|".$content.";", "", $data[1]);
    $data_baru = $data[0].'['.$type.']'.$data[1].'['.$type.']'.$data[2];
    if (! file_exists($this->cacheDir))
...
//    }
  ...
  public function put_all($key, $content)
  {
    $filename_cache = $this->cacheDir . '/' . $key . '.php'; //Cache filename
...
//    print $id_privilege."|".$privilege.";";
//    print_r($data[1]);
//    print strpos($id_privilege."|".$privilege.";", $id_privilege."|".$privilege.";");
//    die;
    return strpos($data[1], $id_privilege."|".$privilege.";");
  }
  
  function cek_module($name){
    $data_json = $this->get("module_status");
    $data_banding = json_decode($data_json);
    if(property_exists($data_banding, $name)){
      return $data_banding->{$name};
    }
    else{
      return false;
    }
